...
OPENROUTER_API_KEY = "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"
...
class DrawingPage(ctk.CTkFrame):
    ...
    def create_canvas(self):
        ...
        try:
            bg_img = tk.PhotoImage(file="C://Users//amaln//Downloads//CALCISKETCH//Assets//image_1.png")
    ...
    def create_toolbar(self):
        toolbar_bg = Button(
            self,
            image=self.load_image("C://Users//amaln//Downloads//CALCISKETCH//Assets//button_1.png"),
        ...
        for i, color in enumerate(colors):
            Button(
                self,
                image=self.load_image(f"C://Users//amaln//Downloads//CALCISKETCH//Assets//button_{i+2}.png"),
                borderwidth=0, highlightthickness=0,
                command=lambda c=color: self.change_color(c), relief="flat"
            ).place(x=x_positions[i], y=937.0, width=63.0, height=63.0)

        Button(
            self,
            image=self.load_image("C://Users//amaln//Downloads//CALCISKETCH//Assets//button_8.png"),
            borderwidth=0, highlightthickness=0,
            command=self.use_eraser, relief="flat"
        ).place(x=695.0, y=928.0, width=83.0, height=83.0)

        Button(
            self,
            image=self.load_image("C://Users//amaln//Downloads//CALCISKETCH//Assets//button_9.png"),
            borderwidth=0, highlightthickness=0,
            command=self.use_brush, relief="flat"
        ).place(x=830.0, y=934.0, width=69.0, height=69.0)

        Button(
            self,
            image=self.load_image("C://Users//amaln//Downloads//CALCISKETCH//Assets//button_12.png"),
            borderwidth=0, highlightthickness=0,
            command=self.save_work, relief="flat"
        ).place(x=1435.0, y=938.0, width=68.0, height=68.0)

        Button(
            self,
            image=self.load_image("C://Users//amaln//Downloads//CALCISKETCH//Assets//button_11.png"),
            borderwidth=0, highlightthickness=0,
            command=self.clear_canvas, relief="flat"
        ).place(x=1300.0, y=938.0, width=68.0, height=68.0)

        Scale(self, from_=1, to=10, orient=HORIZONTAL, command=self.change_size).place(
            x=950, y=938.0, width=300, height=50
        )

        Button(
            self,
            image=self.load_image("C://Users//amaln//Downloads//CALCISKETCH//Assets//button_13.png"),
            borderwidth=0, highlightthickness=0,
            command=self.load_work, relief="flat"
        ).place(x=1550.0, y=938.0, width=68.0, height=68.0)

        Button(
            self,
            image=self.load_image("C://Users//amaln//Downloads//CALCISKETCH//Assets//button_10.png"),
            borderwidth=0, highlightthickness=0,
            command=self.on_calculate_click, relief="flat"
    ...
    def recognize_expression_openrouter(self, image_path):
        ...
        for model in models:
            try:
                        ...
                        "messages": [
                            {
                                ...
                                "content": [
                                    ...
                                    },
                                    {
                                        "type": "text",
                                        "text": (
                                            "This image contains a handwritten mathematical expression. "
                                            "Extract and return only the mathematical expression using standard symbols "
                                            "such as ∫, ×, ², ³, and fractions. No LaTeX. No explanation. Just the expression."
                                        )
                                    }
                                ]
                            }
                ...
                if response.status_code == 429:
                    last_error = f"Rate limit hit on {model}, trying next..."
                    continue
                ...
                if "error" in data:
                    last_error = f"Error from {model}: {data['error']}"
                    continue

                parsed = data["choices"][0]["message"]["content"].strip()
                if parsed:
                    return parsed

            except requests.exceptions.HTTPError:
                last_error = f"HTTP error on model {model}"
                continue
            except Exception as e:
                last_error = str(e)
                continue
    ...
    def evaluate_expression(self):
        ...
        try:
            ...
            # --- OCR via OpenRouter ---
            try:
                parsed_expression = self.recognize_expression_openrouter(temp_image_path)
            except requests.exceptions.ConnectionError:
                messagebox.showerror("OCR Error", "Could not connect to OpenRouter.\nCheck your internet connection.")
                return
            except requests.exceptions.Timeout:
                messagebox.showerror("OCR Error", "OpenRouter request timed out. Please try again.")
                return
            except requests.exceptions.HTTPError as e:
                messagebox.showerror("OCR Error", f"OpenRouter returned an error: {e}\n\nCheck that your API key is valid.")
                return
            ...
            if not parsed_expression:
                messagebox.showwarning("OCR Warning", "No expression detected. Please redraw and try again.")
                return
...
"""
                ...
                messagebox.showerror("Ollama Error", f"Failed to get response from Ollama:\n{str(e)}")
                return
